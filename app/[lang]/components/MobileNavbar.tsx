...
export default function MobileNavbar({ lang }: { lang: Locale }) {
    ...
    <div className="relative mb-3 h-[40px] w-full">
      {showSearch && (
        <input
          ...
          className="h-full w-full rounded-full border border-black px-4 py-2 text-black placeholder-black focus:outline-none"
        />
      )}
      {showSearch && query && (
        <div
  ...
  const renderSearchDropdown = () =>
    ...
    showSearch && (
      <ul
        ref={dropdownRef}
        className="max-h-64 overflow-y-auto rounded-md border border-black bg-white p-2 text-black shadow-lg"
      >
  ...
  return (
    <nav className="fixed top-0 z-50 w-full bg-white p-3 text-black shadow-md">
        ...
        <ul className="mt-3 flex flex-col gap-3 text-lg text-black">
          ...
          <li>
            <Link
              href={`/${lang}/ai`}
              onClick={handleNavClick}
              className={`${activeLink === 'Technology' ? 'font-bold' : ''}`}
            >
