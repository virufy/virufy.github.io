...
const SevenNewsClient = ({ content }: Props) => {
  const { newsCards } = content;
  ...
  const filteredAndSortedCards = useMemo(() => {
    ...
    return result;
  }, [newsCards, selectedYear, selectedPill, sortBy]);
  ...
  return (
    <main>
      <section className="relative w-full shadow-xl">
        ...
        {/* Hero content */}
        <div className="relative mt-2 px-4 pb-12 pt-12 text-center sm:mt-3 sm:px-6 sm:pb-16 sm:pt-16 md:mt-4 md:pb-20 md:pt-20 lg:py-28">
          {/* Tag pill */}
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-2">
            <div className="inline-flex rounded-full border border-gray-300 bg-white p-2 shadow-[0_8px_20px_-4px_rgba(0,0,0,0.15)] ring-gray-200">
              {content.pillSelector.options.map((option) => (
                <button
                  key={option.id}
                  onClick={() => setSelectedPill(option.id)}
                  className={`font-inter rounded-full px-6 py-3 text-base font-medium leading-[24px] tracking-[-0.31px] transition ${
                    selectedPill === option.id
                      ? 'scale-105 bg-gradient-to-r from-blue-600 to-teal-500 text-white shadow-[0_8px_20px_rgba(0,0,0,0.12)]'
                      : 'bg-white text-[#2C3540] hover:bg-gray-200'
                  }`}
                >
                  {option.label}
                </button>
